...
class Sequence_Data:
    ...
    def get_r_target(self, sample_index):
        return self.r_target[sample_index]

    def get_p_t(self, sample_index):
        return self.p_t[sample_index]

    def get_p_next(self, sample_index):
        return self.p_next[sample_index]
    ...
    def get_samples(self, sample_indices):
        samples = {
            'r_target': torch.zeros([len(sample_indices), self.r_size], dtype=torch.float32, device=self.device),
            'p_t':      torch.zeros([len(sample_indices), self.p_size], dtype=torch.float32, device=self.device),
            'p_next':   torch.zeros([len(sample_indices), self.p_size], dtype=torch.float32, device=self.device)
        }
