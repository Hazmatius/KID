...
class StateSeqModel(nn.Module):
    ...
    def forward(self, p_t, p_target):
        p_next = torch.cat([p_t, p_target], 1)
        p_next = self.prelu(self.bn1(self.fc1(p_next)))
        p_next = self.prelu(self.bn2(self.fc2(p_next)))
        p_next = self.tanh(self.bn3(self.fc3(p_next)))
        return p_next
